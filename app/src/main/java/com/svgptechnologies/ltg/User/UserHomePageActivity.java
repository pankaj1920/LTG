...
import com.svgptechnologies.ltg.Json.UserJson.GetDriverLocation.GetDriverLocationData;
import com.svgptechnologies.ltg.Json.UserJson.GetDriverLocation.GetDriverLocationResponse;
...
public class UserHomePageActivity extends AppCompatActivity implements NavigationView.OnNavigationItemSelectedListener,
    ...
    Marker mCurrLocationMarker, DropLocationMarker, DriverMarker;
    ...
    protected void onCreate ( Bundle savedInstanceState ) {
        ...
        UserBtnBookLater = findViewById ( R.id.UserBtnBookLater );
...
//    }
    ...
    public void getDriverLocation ( ) {
        ...
        UserLoginData loginData = UserSharePrefManager.getInstance ( UserHomePageActivity.this ).getUserDetail ( );
        String UserId = loginData.getMobile ( );

        LTGApi ltgApi = BaseClient.getBaseClient ( ).create ( LTGApi.class );
        Call<GetDriverLocationResponse> call = ltgApi.getDriverLocationn ( UserId );

        call.enqueue ( new Callback<GetDriverLocationResponse> ( ) {
            @Override
            public void onResponse ( Call<GetDriverLocationResponse> call, Response<GetDriverLocationResponse> response ) {

                GetDriverLocationResponse driverLocationResponse = response.body ( );
                String Staus = driverLocationResponse.getStatus ( );

                if ( response.isSuccessful ( ) && HttpURLConnection.HTTP_OK == response.code ( ) && Staus.equals ( "1" ) ) {

                    List<GetDriverLocationData> driverLocationData = driverLocationResponse.getData ( );

                    for (GetDriverLocationData data : driverLocationData) {

                        DriverLat = Double.parseDouble ( data.getDriver_lat ( ) );
                        DriverLang = Double.parseDouble ( data.getDriver_lang ( ) );

                        String trip_status = data.getTrip_status ( );

                        if ( trip_status.equals ( "on-going" ) ) {

                            stopthread = false;

                            UserPickupMarker.setVisibility ( View.GONE );
                            userPickupLocation.setVisibility ( View.GONE );

                            LatLng driverLatLang = new LatLng ( DriverLat, DriverLang );

                            MarkerOptions markerOptions = new MarkerOptions ( );
                            markerOptions.position ( driverLatLang );
                            ...
                            Toast.makeText ( UserHomePageActivity.this, "Driver Lat " + DriverLat.toString ( ), Toast.LENGTH_SHORT ).show ( );
                            //  Toast.makeText ( UserHomePageActivity.this, "Driver Lang " + DriverLang.toString ( ), Toast.LENGTH_SHORT ).show ( );
                    ...
                    Toast.makeText ( UserHomePageActivity.this, "Get DriverLocation UnSucess", Toast.LENGTH_SHORT ).show ( );
                }
            }
    ...
    public void startGetDriverLocationThread ( ) {

        stopthread = false;

        GetDriverLocationThread getDriverLocationThread = new GetDriverLocationThread ( );
        new Thread ( getDriverLocationThread ).start ( );
        //     Toast.makeText ( this, "Thread Started", Toast.LENGTH_SHORT ).show ( );
    }

    public void stopGetDriverLocationThread ( ) {
    ...
    class GetDriverLocationThread implements Runnable {
